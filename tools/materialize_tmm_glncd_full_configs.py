#!/usr/bin/env python3
"""Materialize TMM GL-NCD full and ablation configs for cloud training."""
...
OUT_DIR = PROJECT_ROOT / "config" / "tmm_glncd_20260513"
RUN_ROOT = "checkpoints/tmm_glncd_20260513"
...
def v_uniform_target(cfg: Dict[str, Any], spec: Dict[str, Any]) -> None:
    _set_glncd(cfg, spec)
    _set(cfg, ("training", "neighborhood_consistency", "teacher_target"), "uniform")


VARIANTS: Dict[str, List[Tuple[str, Mutator, str]]] = {
    "atrw": [
        ("glncd_full", v_full, "core ATRW validation of GL-NCD"),
        ("glncd_global_only", v_global_only, "ATRW support ablation without stripe-local neighborhood"),
    ],
    "leopard": [
        ("glncd_full", v_full, "full global-local neighborhood consistency"),
        ("glncd_global_only", v_global_only, "remove local stripe neighborhood term"),
        ("glncd_local_only", v_local_only, "remove global embedding neighborhood term"),
        ("glncd_no_hard_negative", v_no_hard_negative, "remove hard-negative suppression"),
        ("glncd_uniform_target", v_uniform_target, "replace teacher neighborhood distribution with uniform positives"),
    ],
    "whaleshark": [
        ("glncd_full", v_full, "full global-local neighborhood consistency"),
        ("glncd_global_only", v_global_only, "remove local stripe neighborhood term"),
    ...
    "gzgc_zebra": [
        ("glncd_full", v_full, "appendix full GL-NCD on existing cross-species zebra evidence"),
        ("glncd_global_only", v_global_only, "appendix support ablation without stripe-local neighborhood"),
    ],
}
...
def materialize() -> List[Dict[str, Any]]:
    ...
    for dataset, spec in DATASETS.items():
        ...
        for variant, mutator, rationale in VARIANTS[dataset]:
            ...
            mutator(cfg, spec)
            cfg["tmm_glncd_20260513"] = {
                "dataset": dataset,
                "variant": variant,
                "priority": spec["priority"],
                "rationale": rationale,
                "algorithm_change": "GL-NCD train-time global-local neighborhood consistency",
